...
If multiple questions are visible, answer all of them concisely."""
    ...
    def get_all_available_models(self) -> list:
        """Get all available models from OpenAI API"""
        if not self.client:
            return []
        ...
        try:
            ...
            for model in models_response.data:
                model_id = model.id
                # Filter for chat completion models (exclude embeddings, whisper, etc.)
                if any(prefix in model_id for prefix in ['gpt-', 'text-davinci', 'o1-']):
            ...
            sorted_models.extend(sorted(all_models))
            
            self.logger.info(f"Found {len(sorted_models)} available models from API")
    ...
    def get_working_models(self) -> list:
        ...
        # First, get all available models from the API
        all_models = self.get_all_available_models()
        working_models = []
        
        self.logger.info(f"Testing {len(all_models)} models for compatibility...")
        
        for i, model in enumerate(all_models):
            try:
                self.logger.info(f"Testing model {i+1}/{len(all_models)}: {model}")
                ...
                if "model" in error_msg and ("does not exist" in error_msg or "not found" in error_msg):
                    ...
                    self.logger.warning(f"Rate limit hit while testing {model}, adding to working list anyway")
                    working_models.append(model)  # Assume it works if rate limited
                else:
        ...
        self.logger.info(f"Found {len(working_models)} working models: {working_models}")
